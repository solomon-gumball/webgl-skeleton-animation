...
class ThreeJsWriter(object):
    ...
    def _allMeshes(self):
        if not hasattr(self, '__allMeshes'):
            ...
            self.__allMeshes = [ls(type='mesh')[0]]
    ...
    def _exportFaces(self, mesh):
        ...
        for face in mesh.faces:
            ...
            self.faces += map(lambda x: x + self.verticeOffset, face.getPolygonTriangleVertices())
            if self.options['materials']:
                if hasMaterial:
    ...
    def _exportKeyframeAnimations(self):
        ...
        for joint in ls(type='joint'):
            hierarchy.append({
                "parent": i,
                "keysFrames": self._getKeyframes(joint, frameRate)
            })
            i += 1
...
ThreeJsWriter().write('~/Desktop/hello.json', 'normals uvs faces vertices bones 3', 'what')